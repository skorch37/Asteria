...
import static handling.MapleServerHandler.log;
...
// <editor-fold defaultstate="collapsed" desc="Packet Log Implementation">
    ...
    public void exceptionCaught(final IoSession session, final Throwable cause) throws Exception {
        ...
        Logger.log("[Exception-caught]: " + client + "\n\n exception information: \n" + cause.getMessage());
    }
        ...
        switch (header) {
            case LOGIN_REDIRECTOR:
                if (!ServerConstants.Redirector) {
                    ...
                    switch (type) {
                        case 1: //test login
                            System.out.println("Redirector test login has been accepeted!");
                            //if(c.login(username, password, false) == 0)
                                //c.getSession().write(RedirectorPacket.getRedirectorAuthSuccessRequest()); //Success
                            //else
                                //c.getSession().write(RedirectorPacket.getRedirectorAuthFailRequest()); //Fail (for whatever reason there is)
                            break;
                        case 2:
                            System.out.println("Redirector real login has been accepeted!");
                            boolean isBanned = c.hasBannedIP() || c.hasBannedMac();
                            if(c.login(username, password, isBanned) == 0) 
                            {
                                c.getSession().write(LoginPacket.getAuthSuccessRequest(c));
                                c.loginData(username);
                            }
                            break;
                        default: //Possible we ip ban here, packet editing definitely
                            System.out.println("Detected invalid Redirector State from: " + c.getSessionIPAddress());
                            c.getSession().close();
                            break;
                    }                   
                }
...
//                System.out.println("[CLIENT ERROR] Error Code " + errortype + " Type " + type_str + "\r\n\t[Data] Length " + data_length + " [" + SendPacketOpcode.getOpcodeName(opcodeheader) + " | " + opcodeheader + "]\r\n" + HexTool.toString(slea.read((int) slea.available())));
            ...
            case PARTY_OPERATION:
                PartyHandler.PartyOperation(slea, c, c.getPlayer());
                break;
...
//                break;
            ...
            default:
                ...
                Logger.log("[UNHANDLED] Recv [" + header.toString() + "] found");
                break;
        }
    }
