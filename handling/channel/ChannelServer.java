...
public class ChannelServer {
    ...
    public void decreaseVermins() {
        for (MapleCharacter chr : players.getAllCharacters()) {
            if (chr.verifyAccMins(chr.getClient()) == 0 && chr.getVerifyStatus(chr.getClient()) == 1) {
                chr.resetVerifyStatus(chr.getClient());
                return;
            }
            if (chr.verifyAccMins(chr.getClient()) == 0) {
                return;
            }
            int currentVermins = 0;
            try {
                Connection con = DatabaseConnection.getConnection();
                ResultSet rs;
                try (PreparedStatement psa = con.prepareStatement("SELECT vermins FROM accounts WHERE name = ?")) {
                    psa.setString(1, chr.getClient().getAccountName());
                    rs = psa.executeQuery();
                    if (rs.next()) {
                        currentVermins = rs.getInt("vermins");
                    }
                }
                try (PreparedStatement ps = con.prepareStatement("UPDATE accounts SET vermins = ? WHERE vermins > 0")) {
                    ps.setInt(1, currentVermins - 1);
                    ps.executeQuery();
                    ps.close();
                }
