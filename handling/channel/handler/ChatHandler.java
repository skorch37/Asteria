...
import client.anticheat.CursesContainer;
...
public class ChatHandler {
                    ...
                    if (target != null) {
                        if (target.getMessenger() == null) {
                            if (!target.isIntern() || c.getPlayer().isIntern()) {
                                ...
                                target.getClient().getSession().write(CField.messengerInvite(c, c.getPlayer().getName(), messenger.getId()));
                            } else {
    ...
    public static void Command(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (mode) {
            ...
            case 6: { // Whisper
                ...
                if (!isApproved(text)) {
                    c.getPlayer().dropMessage(5, "Please talk in a proper language."); //I dont wanna ban them, too lazy to make a new mute function :P
                    return;
                }
                if (ch > 0) {
    ...
    public static boolean isApproved(final String text) {
        return CursesContainer.checkWord(text);
    }
}
