...
//                c.getPlayer().getMap().broadcastMessage(MainPacketCreator.MegidoFlameRe(c.getPlayer().getId(), unk, slea.readInt()));
    ...
    public static void SkillEffect(LittleEndianAccessor slea, MapleCharacter chr) {
        //slea.readInt(); // idk v149.2 
        int skillId = slea.readInt();
        if (skillId >= 91000000 && skillId < 200000000) {
            chr.getClient().getSession().write(CWvsContext.enableActions());
            System.out.println("Skill Effect Return 1");
            return;
        }
        ...
        if ((chr == null) || (skill == null) || (chr.getMap() == null)) {
            System.out.println("Skill Effect Return 2");
            return;
        }
...
//                    c.getSession().write(AngelicPacket.showRechargeEffect());
    ...
    public static void DropMeso(int meso, MapleCharacter chr) {
        int vermins = chr.verifyAccMins(chr.getClient());
        if ((!chr.isAlive()) || (meso < 10) || (meso > 50000) || (meso > chr.getMeso())) {
            chr.getClient().getSession().write(CWvsContext.enableActions());
            return;
        }
        if (ServerConstants.accountSecurity && !chr.getClient().hasSameIP(chr.getClient().getAccountName())
                && chr.getClient().hasSameIP(chr.getClient().getAccountName())) {
            chr.getClient().getSession().write(CWvsContext.broadcastMsg(-1, 
