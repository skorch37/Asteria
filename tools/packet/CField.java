...
public class CField {
    ...
    public static byte[] getChannelChange(MapleClient c, int port) {
        ...
        mplew.write(1);
        mplew.write(ServerConstants.GMS_IP);
    ...
    public static byte[] getCharInfo(MapleCharacter chr) {
        ...
        mplew.writeInt(0);
        mplew.write(1);
        mplew.writeShort(ServerConfig.BoxOnStart ? ServerConfig.content.length : 0);
        
        // Little Box pop up
        if (ServerConfig.BoxOnStart) {
        mplew.writeMapleAsciiString(ServerConfig.BoxTitle); // box title
            for (String content : ServerConfig.content) {
                System.out.println("Box Content "+ content);
                mplew.writeMapleAsciiString(content); // box content
            }
        }
        // End of little box
    ...
    public static byte[] changeHawtBackgroundChannel() {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        mplew.writeShort(SendPacketOpcode.CHANGE_BACKGROUND.getValue());
        mplew.write(HexTool.getByteArrayFromHexString("05 05 00 32 32 35 35 36 00 05 00 32 32 35 35 36 00 05 00 32 32 35 35 36 00 05 00 32 32 35 35 36 00 05 00 32 34 30 36 38 00"));

        return mplew.getPacket();
    }   
     
    public static byte[] changeHawtBackgroundMap() {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
   ...
   public static byte[] getUpdateEnvironmentMapChange(MapleMap map) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
...
//        }
        ...
        if (buffvaluenew.isEmpty()) {
            ...
            for (Pair i : buffvaluenew) {
                if (((Integer) i.right) == 4) {
                    mplew.writeInt(((Integer) i.left));
                } else if (((Integer) i.right) == 2) {
                    mplew.writeShort(((Integer) i.left).shortValue());
                } else if (((Integer) i.right) == 1) {
                    mplew.write(((Integer) i.left).byteValue());
                } else if (((Integer) i.right) == 0) {
                    mplew.writeZeroBytes(((Integer) i.left));
                }
            }
        }
        mplew.writeZeroBytes(36);//v148
        ...
        //mplew.writeZeroBytes(6); // removed v149.2 ?
    ...
    public static byte[] getScrollEffect(int chr, Equip.ScrollResult scrollSuccess, boolean legendarySpirit, int item, int scroll) {
        ...
        mplew.writeInt(0); // scroll
        mplew.writeInt(0); // item
        mplew.writeInt(0);
        mplew.write(0);
        mplew.write(0);
    ...
    public static byte[] followEffect(int initiator, int replier, Point toMap) {
        ...
        //mplew.writeLong(0); // removed in v148.2
        if (replier == 0) {
    ...
    public static byte[] movePlayer(int cid, List<LifeMovementFragment> moves, Point startPos) {
        ...
        mplew.writeInt(0);
        mplew.writePos(startPos);
        mplew.writeShort(0); //that's being change too in GMS
        mplew.writeShort(0); //that's being change too in GMS
        PacketHelper.serializeMovementList(mplew, moves);
    ...
    public static byte[] addAttackInfo(int type, int cid, int tbyte, int skill, int level, int display, byte speed, List<AttackPair> damage, int lvl, byte mastery, byte unk, int charge, Point pos, int ultLevel) {
      ...
      //  System.out.println(tbyte + " - tbyte");
        mplew.writeShort(lvl); // Changed to short in v149.3 ?
        if ((skill > 0) || (type == 3)) {
            //mplew.write(level); // Deleted in v148.2 I think (Need to check) // skill level
            if (level > 0) {
                mplew.writeInt(skill);
            }
        ...
        mplew.write(unk);//always 0? changed to short in v148.2
        if ((unk & 2) != 0) {
            mplew.writeInt(0);
            mplew.writeInt(0);
        }
...
//            mplew.write(0);//removed 143 or other0
    ...
    public static byte[] messengerInvite(MapleClient c, String from, int messengerid) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        mplew.writeShort(SendPacketOpcode.MESSENGER.getValue());
        mplew.write(3);
        mplew.writeMapleAsciiString(from);
        mplew.write(c.getChannel());
...
//            mplew.write(UserSlot);
//            mplew.write(7);//was2
              mplew.write(HexTool.getByteArrayFromHexString("1C 01 07"));
