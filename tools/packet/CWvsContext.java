...
//                case JOB:
                ...
                case MESO:
                    ...
                    //mplew.writeLong((statupdate.getValue()).longValue()); // v149.3
                    break;
                case PET:
        ...
        mplew.write(0);
        mplew.write(0);
        //mplew.writeZeroBytes(40); // for no dcs
    ...
    public static byte[] craftMessage(String msg, Item item) {
        MaplePacketLittleEndianWriter p = new MaplePacketLittleEndianWriter();

        p.writeShort(SendPacketOpcode.SERVERMESSAGE.getValue());
        p.write(22);
        p.writeMapleAsciiString(msg);
        if (item != null) {
            PacketHelper.addItemInfo(p, item);
        }
        return p.getPacket();
    }
    ...
    public static byte[] getTopMsg(String msg, int type, int idk) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        mplew.writeShort(type <= 0 ? SendPacketOpcode.TOP_MSG.getValue() : SendPacketOpcode.NEW_TOP_MSG.getValue());
        if (type > 0) {
            mplew.writeInt(3);
            if (type == 20) {
                mplew.writeInt(type);
                mplew.writeInt(15); // idk
            } else {
                mplew.writeInt(type);
                mplew.writeInt(idk);
            }
    ...
    public static byte[] sendWelcomeBack(MapleCharacter chr) { // When you back(after year or later) in gms it send you special top message (very cool)
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        mplew.writeShort(SendPacketOpcode.NEW_TOP_MSG.getValue());
        mplew.writeInt(3);
        mplew.writeInt(20); // type I think
        mplew.writeInt(15); // idk
        mplew.writeMapleAsciiString("Welcome to Maple World, " + chr.getName() + "! We're one big family!");
...
//    }
    ...
    public static class BuddylistPacket {
        ...
        public static byte[] updateBuddylist(Collection<BuddylistEntry> buddylist) {
            return updateBuddylist(buddylist, 10);
        }
        ...
        public static byte[] updateBuddylist(Collection<BuddylistEntry> buddylist, int deleted) {
            ...
            for (BuddylistEntry buddy : buddylist) {
                ...
                mplew.writeAsciiString(buddy.getGroup(), 18); // 18 in v148.2
            }
        ...
        public static byte[] requestBuddylistAdd(int cidFrom, String nameFrom, int levelFrom, int jobFrom, int buddylist) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

            mplew.writeShort(SendPacketOpcode.BUDDYLIST.getValue());
            // v148.2 packet data
            // 09 36 79 85 00 0A 00 64 65 61 6E 31 32 32 32 32 32 98 00 00 00 0A 02 00 00 00 00 00 00 36 79 85 00 64 65 61 6E 31 32 32 32 32 32 00 74 72 01 02 00 00 00 47 72 6F 75 70 20 55 6E 6B 6E 6F 77 6E 00 00 00 00 00 00
            mplew.write(9);// 9 in v148.2
            //mplew.write(1); // removed in v148.2
            ...
            mplew.writeInt(cidFrom);
            mplew.writeAsciiString(nameFrom, 11); // 11 in v148.2 (I think)
            mplew.writeShort(0);
            mplew.write(1); // was 0
            mplew.writeInt(buddylist); // Buddylist size
            //mplew.write(0); // removed in v148.2
            mplew.writeAsciiString(jobFrom == 0 ? "Group Unknown" : "ETC", 18);
            mplew.write(0);
...
//        mplew.writeZeroBytes(6);
    ...
    public static class PartyPacket {
        ...
        public static void addPartyStatus(int forchannel, MapleParty party, MaplePacketLittleEndianWriter lew, boolean leaving, boolean exped) {
            ...
            for (MaplePartyCharacter partychar : partymembers) {
                ...
                lew.writeInt(Integer.MAX_VALUE);
            }
            for (int i = 0; i < 6; i++) {
                lew.writeInt(0);
            }
    ...
    public static class GuildPacket {
        ...
        public static byte[] GuildCreated(String guildname) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        public static byte[] guildContribution(int gid, int cid, int c) {
            ...
            mplew.writeInt(cid);
            mplew.writeInt(c);
            mplew.writeInt(c); //v149
            mplew.writeInt(0); // idk
            mplew.writeLong(0); // date
        ...
        public static byte[] SearchResult(MapleClient c, Map<Integer, MapleGuild> sorted) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

            mplew.writeShort(0x100); //header
            
            mplew.writeInt(sorted.size());
            if (sorted.size() > 0) {
                
            for (MapleGuild gs : sorted.values()) {
            mplew.writeInt(gs.getId()); // guild id 
            mplew.writeInt(gs.getLevel()); // guild level
            mplew.writeMapleAsciiString(gs.getName());
            mplew.writeMapleAsciiString(gs.getLeader(c).getName());
            mplew.writeInt(gs.getMembers().size()); // members
            mplew.writeInt(gs.getAveregeLevel()); // averege level
            }
...
//            mplew.write(0);//v112
        ...
        public static byte[] giveBuff(int buffid, int bufflength, Map<MapleBuffStat, Integer> statups, MapleStatEffect effect) {
            ...
            mplew.writeShort(0);
             ...
             //make sure no dc incase not enough length
            ...
            mplew.writeZeroBytes(69); // for no dcs
            return mplew.getPacket();
        }
...
//            mplew.write(0);//v112
        ...
        public static byte[] cancelForeignDebuff(int cid, MapleDisease mask) {
            ...
            PacketHelper.writeSingleMask(mplew, mask);//52 bytes
            //mplew.write(3);
            mplew.write(1);
            //mplew.write(0);//v112
