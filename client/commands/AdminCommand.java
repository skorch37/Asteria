...
public class AdminCommand {
    ...
    public static class TopMsg extends CommandExecute {

        @Override
        public int execute(MapleClient c, String[] splitted) {
            String text = splitted[1];
            String type = splitted[2];
            String idk = splitted[3];
            
            if (text.length() <= 0) {
                c.getPlayer().dropMessage(5, "The text must be more than 0 letters !");
                return 0; 
            }
            if (type == null || idk == null || Integer.parseInt(type) <= 0 || Integer.parseInt(idk) <= 0) {
                c.getPlayer().dropMessage(5, "Sytext error: !topmsg [text] [type] [idk]");
                return 0; 
            }
            c.getSession().write(CWvsContext.getTopMsg(text, Integer.parseInt(type), Integer.parseInt(idk)));
            return 1;
        }
    }
    
        public static class WelcomeBack extends CommandExecute {

        @Override
        public int execute(MapleClient c, String[] splitted) {
            c.getSession().write(CWvsContext.sendWelcomeBack(c.getPlayer()));
            return 1;
        }
    }
