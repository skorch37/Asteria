...
public class GMCommand {
    ...
    public static class maxskills extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
             c.getPlayer().maxSkillsByJob();
             MapleStat[] stats = {MapleStat.LUK, MapleStat.INT, MapleStat.STR, MapleStat.DEX};
             c.getPlayer().updateMultiStats(stats, 32767);
             MapleStat[] hpmp = {MapleStat.HP, MapleStat.MP, MapleStat.MAXHP, MapleStat.MAXMP};
             c.getPlayer().updateMultiStats(hpmp, 30000);
             c.getPlayer().updateSingleStat(MapleStat.FAME, 32767);
             c.getPlayer().updateSingleStat(MapleStat.MESO, Integer.MAX_VALUE);
             stats = null;
             hpmp = null;
             return 1;
        }
