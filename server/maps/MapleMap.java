...
import clientside.MonsterStatus;
...
//        }
        ...
        if (!chr.isClone()) {
            ...
            if (getNumMonsters() > 0 && (mapid == 280030001 || mapid == 240060201 || mapid == 280030000 || mapid == 240060200 || mapid == 220080001 || mapid == 541020800 || mapid == 541010100)) {
                ...
                chr.getClient().getSession().write(CWvsContext.sendWelcomeBack(chr));
        ...
        //chr.getClient().getSession().write(CField.getUpdateEnvironmentMapChange(this));
        //chr.getClient().getSession().write(CField.getMapleEventList());
        List<Triple<String, Integer, Boolean>> backgrounds = new LinkedList<>(); //boolean for randomize
        Triple<String, Integer, Boolean>[] bg = new Triple[]{ //boolean for randomize
        new Triple<>("22556", 0, false),
        new Triple<>("22556", 0, false),
        new Triple<>("22556", 0, false),
        new Triple<>("22556", 0, false),
        new Triple<>("24068", 0, false),
        }; 
        backgrounds.addAll(Arrays.asList(bg));
        chr.getClient().getSession().write(LoginPacket.changeBackground(backgrounds));
