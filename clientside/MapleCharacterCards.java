...
public class MapleCharacterCards {
    ...
    private final List<Pair<Integer, Integer>> skills = new LinkedList();
    ...
    public final void calculateEffects() {
        ...
        if ((deck1amount == 3) && (cardids1.size() == 3)) {
            ...
            for (Integer ii : uid) {
                this.skills.add(new Pair(ii, GameConstants.getSkillLevel(lowD1)));
            }
            this.skills.add(new Pair(CharacterCardFactory.getInstance().getRankSkill(lowD1), 1));
        }
        if ((deck2amount == 3) && (cardids2.size() == 3)) {
            List<Integer> uid = CharacterCardFactory.getInstance().getUniqueSkills(cardids2);
            for (Integer ii : uid) {
                this.skills.add(new Pair(ii, GameConstants.getSkillLevel(lowD2)));
            }
            this.skills.add(new Pair(CharacterCardFactory.getInstance().getRankSkill(lowD2), 1));
        }
    }
