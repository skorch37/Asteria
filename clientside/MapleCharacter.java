...
public class MapleCharacter extends AnimatedMapleMapObject implements Serializable, MapleCharacterLook {
    ...
    private boolean canTrade = true, canDrop = true;
    private Point old;
    ...
    private boolean changed_wishlist, changed_trocklocations, changed_regrocklocations, changed_hyperrocklocations, changed_skillmacros, changed_achievements, changed_moonlight_achievements,
            changed_savedlocations, changed_questinfo, changed_reports, changed_extendedSlots, update_skillswipe;
    public boolean changed_skills;
    /*
    ...
    public static MapleCharacter getDefault(final MapleClient client, final JobType type) {
        ...
        ret.buddylist = new BuddyList((byte) 232);
...
//            }
    ...
    public void saveToDB(boolean dc, boolean fromcs) {
        ...
        try {
            ...
            if (exp > Long.MAX_VALUE || exp < 0) { //otherwise the char won't be saved when his at high lv..
                exp = getNeededExp();
            }
...
//        System.out.println(start + " | " + end);
    ...
    public void resetAP() {
        int stoadd = this.str + this.dex + this.luk + this.int_;
        updateSingleStat(MapleStat.STR, 0);
        updateSingleStat(MapleStat.DEX, 0);
        updateSingleStat(MapleStat.LUK, 0);
        updateSingleStat(MapleStat.INT, 0);
        updateSingleStat(MapleStat.AVAILABLEAP, stoadd);
    }
    ...
    public void updateMultiStats(MapleStat stat[], long newval) {
        updateMultiStats(stat, newval, false);
    }

    public void updateMultiStats(MapleStat stats[], long newval, boolean itemReaction) {
        Map<MapleStat, Long> statup = new EnumMap<>(MapleStat.class);
...
//            client.getSession().write(InventoryPacket.updateSpecialItemUse(item, type == MapleInventoryType.EQUIPPED ? (byte) 1 : type.getType(), this));
    ...
    public final int maxLevel = ServerConstants.maxLevel;
    ...
    public void levelUp() {
        ...
        if (getExp() > getNeededExp() || getExp() < 0) { //Nagative exp fix
            setExp(getNeededExp());
        }
    ...
    public int verifyAccMins(MapleClient c) {
        int vermins = 0;
        try {
            Connection con = DatabaseConnection.getConnection();
            ResultSet rs;
            try (PreparedStatement ps = con.prepareStatement("SELECT vermins FROM accounts WHERE id = ?")) {
                ps.setInt(1, c.getAccID());
                rs = ps.executeQuery();
                if (rs.next()) {
                    vermins = rs.getInt("vermins");
                }
                ps.close();
                rs.close();
                return vermins;
    ...
    public void resetVerifyStatus(MapleClient c) {
        try {
            Connection con = DatabaseConnection.getConnection();
            try (PreparedStatement ps = con.prepareStatement("UPDATE accounts SET DifferentIP = 0 WHERE id = ?")) {
                ps.setInt(1, c.getAccID());
                ps.executeQuery();
                ps.close();
            }
    ...
    public int getVerifyStatus(MapleClient c) {
        int status = 0;
        try {
            Connection con = DatabaseConnection.getConnection();
            ResultSet rs;
            try (PreparedStatement ps = con.prepareStatement("SELECT DifferentIP FROM accounts WHERE id = ?")) {
                ps.setInt(1, c.getAccID());
                rs = ps.executeQuery();
                if (rs.next()) {
                    status = rs.getInt("DifferentIP");
                }
                ps.close();
                rs.close();
                return status;
    ...
    public void dropMessage(int type, String message) {
        switch (type) {
            case -1:
                client.getSession().write(CWvsContext.getTopMsg(message, 0, 0));
                break;
            case -2:
    ...
    public final void maxSkillsByJob() {
        HashMap<Skill, SkillEntry> sa = new HashMap<>();
        for (Skill skil : SkillFactory.getAllSkills()) {
            if (GameConstants.isApplicableSkill(skil.getId()) && skil.canBeLearnedBy(getJob()) && !skil.isInvisible()) { //no db/additionals/resistance skills
                sa.put(skil, new SkillEntry((byte) skil.getMaxLevel(), (byte) skil.getMaxLevel(), SkillFactory.getDefaultSExpiry(skil)));
            }
        }
        changeSkillsLevel(sa);
