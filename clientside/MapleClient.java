...
public class MapleClient implements Serializable {
   ...
   public void loginData(String login) {
    ...
    public boolean hasSameIP(String username) {
        String LastIP = "";
        String currentIP = getSessionIPAddress();
        Connection con = DatabaseConnection.getConnection();
        try (PreparedStatement ps = con.prepareStatement("SELECT * FROM accounts WHERE name = ?")) {
            ...
            System.out.println("[Nex-bot]: Unable to execute the command (MapleClient => hasSameIP).");
            return currentIP.equals(LastIP);
        }
    }

    public void setVerifyStatus(String username, int status) {
        Connection con = DatabaseConnection.getConnection();
        try (PreparedStatement ps = con.prepareStatement("UPDATE accounts SET DifferentIP = ? WHERE name = ?")) {
            ps.setInt(1, status);
        ...
        try (PreparedStatement psa = con.prepareStatement("UPDATE accounts SET SessionIP = " + getSessionIPAddress() + " WHERE name = ?")) {
   ...
   public int login(String login, String pwd, boolean ipMacBanned) {
                        ...
                        if (banned > 0 && gm) {
                            ...
                            if (loginstate > MapleClient.LOGIN_NOTLOGGEDIN) { // already loggedin
                                ...
                                if (LoginCryptoLegacy.isLegacyPassword(passhash) && LoginCryptoLegacy.checkPassword(pwd, passhash)) {
                                    ...
                                    loginok = 0;
                                    updatePasswordHash = true;
                                } else if (passhash.equals(pwd)) { // Plain Password Hash It :)
                                    loginok = 0;
                                    updatePasswordHash = true;
    ...
    public boolean CheckSecondPassword(String in) {
        ...
        if (LoginCryptoLegacy.isLegacyPassword(secondPassword) && LoginCryptoLegacy.checkPassword(in, secondPassword)) {
            ...
            allow = true;
            updatePasswordHash = true;
        } else if (in.equals(secondPassword)) { // Plain Password Hash It :)
            allow = true;
            updatePasswordHash = true;
    ...
    public byte getAccountWorld(String accountname) {
        if (!hasSameIP(accountname)) {
            setVerifyStatus(accountname, 1);
        } else {
            setVerifyStatus(accountname, 0);
        }
        byte ret = 0;
        try {
            Connection con = DatabaseConnection.getConnection();
